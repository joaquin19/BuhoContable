...
import { OrderHeader } from '@app/core/models';
...
export class OrderHeaderService {
  ...
  private endpointOrderHeader: string;
  ...
  constructor(
    ...
    this.endpointOrderHeader = this.endPoints.getApiOrderHeader;
  }

  getOrders(userName) {
    const endpoint = this.settings.generateEndpoint(this.endpointOrderHeader);
    return this.http.get<OrderHeader[]>(`${endpoint}/GetOrders?userName=${userName}`);
  }

  getOrderById(orderId) {
    const endpoint = this.settings.generateEndpoint(this.endpointOrderHeader);
    return this.http.get<OrderHeader[]>(`${endpoint}/GetOrderById?orderId=${orderId}`);
  }

  getOrdersByCustomerId(customerId, currencyId, startDate, endDate, saleSupportHeaderId) {
    const endpoint = this.settings.generateEndpoint(this.endpointOrderHeader);
    return this.http.get<OrderHeader[]>(`${endpoint}/getOrdersByCustomerId?customerId=${customerId}`
      + `&currencyId=${currencyId}&startDate=${startDate}&endDate=${endDate}&saleSupportHeaderId=${saleSupportHeaderId}`);
  }

  saveOrder(form: FormData) {
    const endpoint = this.settings.generateEndpoint(this.endpointOrderHeader);
    return this.http.post<OrderHeader>(`${endpoint}/SaveOrder`, form);
  }

  updateOrder(form: FormData) {
    const endpoint = this.settings.generateEndpoint(this.endpointOrderHeader);
    return this.http.put<OrderHeader[]>(`${endpoint}/UpdateOrder`, form);
  }

  deleteOrder(orderId, deleteBy) {
    const endpoint = this.settings.generateEndpoint(this.endpointOrderHeader);
    return this.http.delete<OrderHeader>(`${endpoint}/DeleteOrder?orderId=${orderId}&deleteBy=${deleteBy}`);
  }

  getDocumentDefinition(cols: any, data: any, dataParent: any) {
