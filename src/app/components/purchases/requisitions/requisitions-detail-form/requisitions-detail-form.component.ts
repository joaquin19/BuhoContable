...
import {
  AlertMessageService, SessionService, RequisitionHeaderService, RequisitionDetailService,
  PlantService, SupplierRecordService, TaxService
...
export class RequisitionsDetailFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cols = [
      ...
      { field: 'unitPrice', header: 'PRECIO UNITARIO', width: 120, textAlign: 'right', format: '$0,0.00' },
      { field: 'subTotal', header: 'PRECIO TOTAL', width: 120, textAlign: 'right', format: '$0,0.00' },
      { field: 'total', header: 'PRECIO TOTAL', width: 120, textAlign: 'right', format: '$0,0.00', visible: false }
    ];
    this.showForm();
  }
  ...
  showForm() {
    ...
    this.getRequisitionById(this.item.id);
    this.getRequisitionDocuments(this.item.id);
  }
  ...
  public downloadPDF() {
    ...
    const requisitionNamePDF = `R${this.requisition.id}(${this.requisition.supplierName})${dateCreateOn[0]}${dateCreateOn[1]}${dateCreateOn[2]}.pdf`;
    const documentDefinition =
      this.requisitionHeaderService.getDocumentDefinition(this.cols, this.listRequisitionDetail,
        this.requisition, this.listTaxesAdded);
