...
import {
  AlertMessageService, PurchaseOrderHeaderService, SessionService, PurchaseOrderDetailService,
...
@Component({
  selector: 'app-reconciliation-authorization-detail-form',
  templateUrl: './reconciliation-authorization-detail-form.component.html',
  styleUrls: ['./reconciliation-authorization-detail-form.component.scss']
})
export class ReconciliationAuthorizationDetailFormComponent implements OnInit {
  ...
  public maxLengthObservationsAuthorization: number;
  ...
  constructor(
    ...
    this.maxLengthObservationsAuthorization = 200;
    this.purchaseOrder = {
      id: 0,
      folio: '',
  ...
  showForm() {
    this.getPurchaseOrderById(this.item.valueId);
    this.getPurchaseOrderDocuments(this.item.valueId);
  }
      ...
      data => {
        switch (authorizationStatusId) {
          case AuthorizationStatus.Authorized:
            this.alertMessageService.successMessage('Orden de Compra Autorizada correctamente.');
            break;
  ...
  calculationTaxes() {
    ...
    console.log(this.listTaxesAdded);
    this.calculationTotal();
  }
